...
const FolderCleateButton: React.FC<FolderCleateButtonProps> = (props) => {
  ...
  return (
    <div onDoubleClick={() => setFolderCreating(false)} className="flex items-center ">
      <button
      ...
      {folderCreating && (
        <div className="flex max-w-6xl  flex-grow relative">
          <input
            ...
            onChange={(e) => setCreateFolderInputValue(e.target.value.replace(/\s+/g, ''))}
          />
          <GiConfirmed onClick={newFolderCreate} className="text-2xl hover:bg-black rounded-full hover:text-white  absolute top-1/4  right-4" />
        </div>
      )}
    </div>
  )
